...
function mh_board_edit_order_columns($columns) {
	...
	$columns["ID"]			= "번호";
	$columns['board_cat']	= '카테고리';
	$columns['title']		= '제목';
	$columns['comments']	= '댓글';
	$columns['mhauthor']	= '작성자';
	$columns['date']		= '작성일';
	$columns['count']		= '조회수';

    return $columns;
}
add_action('manage_board_posts_custom_column', 'mh_board_custom_order_columns', 2);

function mh_board_custom_order_columns($column) {
